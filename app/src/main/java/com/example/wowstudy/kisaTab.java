...
public class kisaTab extends TabActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        {TabHost.TabSpec tabSpec1 = host.newTabSpec("Tab1");
            tabSpec1.setIndicator("징벌 성기사"); // 탭에서 보이는 부분
            tabSpec1.setContent(R.id.tab_view1); // Tab Content
            host.addTab(tabSpec1);

            InputStream inputS = getResources().openRawResource(R.raw.kisadps);
            ivDPS.setImageResource(R.drawable.kisadps);
            try {
        ...
        {TabHost.TabSpec tabSpec2 = host.newTabSpec("Tab2");
            tabSpec2.setIndicator("신성 성기사"); // Tab Subject
            tabSpec2.setContent(R.id.tab_view2); // Tab Content
            host.addTab(tabSpec2);

            InputStream inputSS = getResources().openRawResource(R.raw.kisahill);
            ivTank.setImageResource(R.drawable.kisahill);
            try {
